...
function updateCategoriesUI(categories) {
    ...
    Object.entries(categories).forEach(([categoryId, category]) => {
        ...
        categoryElement.innerHTML = `
            ...
            <div class="services-list" id="services-${categoryId}"></div>
        `;
...
function loadServices(categoryId) {
    ...
    const servicesRef = database.ref(`categories/${categoryId}/services`);
...
function updateServicesUI(categoryId, services) {
    ...
    if (!servicesList) {
        console.error('Services list container not found for category:', categoryId);
        return;
    }
    ...
    Object.entries(services).forEach(([serviceId, service]) => {
        ...
        serviceElement.innerHTML = `
            <div class="service-details">
                <h4>${service[currentLanguage] || service.name_en || service.name_ar || 'Unnamed Service'}</h4>
                <p>${service.price || 0} ${translations[currentLanguage].currency} - ${service.duration || 30} ${translations[currentLanguage].minutes}</p>
            </div>
...
function addService(categoryId, service) {
    showLoader();
    database.ref(`categories/${categoryId}/services`).push(service)
        .then(() => {
            hideLoader();
            showNotification(translations[currentLanguage].successMessage, 'success');
        })
...
function openAddServiceModal(categoryId) {
    ...
    modalForm.innerHTML = `
        ...
        <div class="form-group">
            <label for="service-duration">${translations[currentLanguage].serviceDuration}</label>
            <input type="text" id="service-duration" placeholder="e.g., 30m">
        </div>
        <button type="button" id="save-service">${translations[currentLanguage].save}</button>
    `;
